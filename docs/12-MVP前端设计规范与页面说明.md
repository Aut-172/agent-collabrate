...
- Workflow 按状态机状态和 Intent 级别分布；
- Task 按状态分布、开放任务数和开放工作量；
- 成员当前分配任务、开放工作量、周期内完成数，以及开放工作量 / 每周容量比例；投入程度预设映射为高（Full Time，每周 36 点）、中（Part Time，每周 24 点）、低（Limited，每周 12 点）；
- 最近 30 天 Workflow 创建趋势和 Task 完成趋势（服务端按 UTC 日期聚合，可通过日期范围接口扩展）。
- Leader 还可以从左侧“调用日志”查看项目 AI 调用明细；页面顶部显示成功/失败/运行中调用数、耗时和 Tokens 汇总，单条记录可展开查看请求与 Provider 反馈。

成员统计按负载比例从高到低展示，未设置每周容量的成员排在末尾；统计接口失败时仍保留基础项目概览和当前 Workflow 列表。活动热力图与 Tokens 花费统计不属于当前阶段，待后续补充统一的数据口径后再加入。
...
Design、Spec 和其他未确认的 AI 文档在工作流详情中提供编辑入口。编辑保存会生成新的用户版本，保留原版本记录；确认按钮使用主按钮样式。每份 Markdown 文档提供低强调度的下载按钮，使用下载图标和工具提示，不与生成、编辑、确认等主操作竞争；下载内容在原文末尾附加该文档版本的已确认决策（问题、最终选项和选项键），没有已确认决策时不追加空章节。Build Plan 的批准按钮仅对项目 Leader 显示，普通成员显示“等待 Leader 批准 Build Plan”。
...
Build Plan 审批区域在任务超过粒度阈值时显示“任务拆分辅助”：警告列出涉及的 Task 和建议动作。Leader 可在 Task 创建前勾选至少两个 Task 执行“合并选中 Task”，系统保存新的 Build Plan 版本；也可选择“保留当前拆分”，必须填写简短理由。理由输入框不应默认展开为主操作，提交批准时若仍存在警告同样要求理由。合并后的计划批准并创建任务后，任务包按新任务边界生成初始版本；Task 创建后不再显示计划合并入口，避免修改开发中或已有交付的任务。其他 regenerate 场景替换已有任务包时，页面展示递增版本和旧版本差异。
