...
Build Plan 审批区域在任务超过粒度阈值时显示“任务拆分辅助”：警告列出涉及的 Task 和建议动作。Leader 可勾选至少两个 Task 执行“合并选中 Task”，系统保存新的 Build Plan 版本；也可选择“保留当前拆分”，必须填写简短理由。理由输入框不应默认展开为主操作，提交批准时若仍存在警告同样要求理由。合并后的计划批准并创建任务后，任务包按新任务边界生成；已有任务包被替换时展示递增版本和旧版本差异。
