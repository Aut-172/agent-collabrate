...
### 8.1 Design 与 Spec 的 Prompt 边界

Design 和 Spec 虽然都以 Markdown 保存，但不是同一份内容的不同标题。平台 Prompt 必须为两者指定不同的职责：

- Design 回答为什么做、做什么、边界如何划分、方案如何取舍；不得输出 API 字段、SQL、逐文件修改、Task 或分工。
- Spec 回答系统必须表现出什么行为、契约如何定义、错误如何处理、如何验收；不得重新设计已确认的架构，也不得输出 Task 或分工。
- 两类文档的一级、二级标题统一使用中文，可保留数字编号；标题不得使用 `Decision Summary`、`Scope` 等英文文本。

两类文档都必须引用当前 Code Context，并将未知内容明确写入 Assumptions 或 Open Decisions，不能把推测写成仓库事实。证据引用使用行内代码路径（例如 `pom.xml`）或标准 Markdown 链接，不使用 `【路径】`，避免破坏正文阅读节奏。生成器应使用不同的章节清单和禁止事项，输出校验也应检查章节覆盖、标题语言及越界内容。

Prompt 的篇幅约束采用“定章节 + 分节字数预算 + 条目数/句数约束”，不只依赖模型对 token 数的估算：

- Design 固定 10 个中文章节，正文目标为 3000-4500 个中文字符，绝对不超过 5000 个中文字符；方案架构可以使用 600-850 字，清单型章节限制为 3-12 条。
- Spec 固定 12 个中文章节，正文目标为 3200-4800 个中文字符，绝对不超过 5200 个中文字符；行为场景限制为 4-10 个，验收矩阵最多 12 项。
- 段落通常不超过 3 句，普通条目不超过 2 句；待确认决策每条使用唯一编号 `DEC-001`、`DEC-002`……，最多 4 句，且必须包含问题、至少两个可选项、建议和未确认影响；没有决策时明确写“无待确认决策”。
- 分节预算是内容上限而不是填充目标。同一事实或规则只完整说明一次，其他章节使用编号或证据路径引用，禁止为达到目标字数补写背景。
- `AGENT_MAX_OUTPUT_TOKENS=6000` 仍由 Responses API 的 `max_output_tokens` 提供硬上限；中文字符预算负责在硬上限之前主动收敛内容，避免生成耗时随允许 token 数持续增长。

### 8.2 Task 粒度提示

Build Plan 的默认原则是“一个 Task 对应一个可独立评审、验证和交付的成果”。Change 和小型 Feature 默认优先生成一个端到端 Task；只有存在独立验收成果、明确模块/回滚边界或真实并行价值时才拆分。不得按 Controller/Service/Repository/Test、前端/后端/测试或文件数量机械拆分，也不得为了满足推荐人数而拆分。平台对可疑的过细拆分记录非阻断式警告，由 Leader 在审批时决定合并或保留。
...
当多个成员在技能、职责和限制上都满足任务要求时，Agent 必须横向比较所有候选人的当前负载。负载比例定义为
`workload_ratio = open_effort_points / weekly_capacity_points`，优先选择比例较低者，并在生成多个任务时把前面任务的
`effort_points` 累加到后续比较中。负载均衡只能作为能力匹配后的软优化，不能因为比例更低就把后端任务分给纯前端成员。
