...
仓库提供三服务 Docker Compose 编排：PostgreSQL、Spring Boot 后端，以及提供 Vite 构建产物和 `/api` 反向代理的 Nginx 前端。后端与前端都使用多阶段构建，最终镜像不包含 Maven、Node.js、源码或本地依赖目录。Flyway 随后端启动执行；数据库使用命名卷持久化并应定期备份。生产部署由 `.github/workflows/ci-cd.yml` 在 `main` 通过 CI 后构建并推送阿里云 ACR 镜像，再通过 SSH 将 `compose.prod.yaml`、`Caddyfile` 和镜像环境文件同步到公网 ECS；ECS 上的 `.env` 单独保存数据库、JWT、域名和 Agent/Git 凭证，Caddy 对外提供 80/443 和自动 HTTPS，PostgreSQL、后端及前端只加入 Compose 内网。生产凭证只能通过 GitHub Environment Secrets、ECS 环境文件或密钥管理服务注入，不能写入镜像。具体 ECS 前置条件、Secret 清单和回滚步骤见 [16-GitHub Actions公网部署.md](16-GitHub%20Actions%E5%85%AC%E7%BD%91%E9%83%A8%E7%BD%B2.md)。
