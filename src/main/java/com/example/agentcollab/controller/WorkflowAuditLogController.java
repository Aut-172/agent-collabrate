...
import com.example.agentcollab.service.ProjectAccessService;
...
public class WorkflowAuditLogController {
    private final AuditLogService audits; private final UserService users; private final WorkflowRepository workflows; private final ProjectAccessService access;
    public WorkflowAuditLogController(AuditLogService audits, UserService users, WorkflowRepository workflows, ProjectAccessService access) { this.audits = audits; this.users = users; this.workflows = workflows; this.access = access; }
    @GetMapping
    public Page<AuditLogDtos.AuditLogResponse> list(@PathVariable Long workflowId,
        ...
        access.requireLeader(workflow.getProjectId(), actor);
