...
function DocumentDecisionPanel({items,busy,canResolve,onResolve}){
...
function WorkflowDetail({workflowId,project,user,onBack,onTask,onUpdated}){
  const [workflow,setWorkflow]=useState(null),[documents,setDocuments]=useState([]),[decisions,setDecisions]=useState([]),[context,setContext]=useState(null),[inventory,setInventory]=useState(null),[tasks,setTasks]=useState([]),[audits,setAudits]=useState([])
  ...
  return <>
    ...
    {build&&<BuildPlanSection workflow={workflow} document={build} plan={plan} editing={planEditing} draft={planDraft} busy={busy} canApprovePlan={isLeader} onEdit={()=>{setPlanDraft(formatJson(build.content));setPlanEditing(true)}} onDraft={setPlanDraft} onCancel={()=>setPlanEditing(false)} onSave={savePlan} onApprove={()=>action(`/api/workflows/${workflowId}/approve-plan`,{versionNo:build.versionNo})} onCreateTasks={()=>action(`/api/workflows/${workflowId}/create-tasks`)}/>}
