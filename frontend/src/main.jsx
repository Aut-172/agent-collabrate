...
import {Activity, Bell, CheckCircle2, CircleAlert, Copy, Download, Ellipsis, Eye, FileText, FolderArchive, LayoutDashboard, LogOut, Menu, Pencil, Plus, RefreshCw, UserPlus, Users, Workflow as WorkflowIcon, X, XCircle} from 'lucide-react'
...
const workflowStatusOrder=['INTENT','DESIGN_PROPOSED','DESIGN_CONFIRMED','SPEC_PROPOSED','SPEC_CONFIRMED','BUILD_PLAN_PROPOSED','PLAN_APPROVED','TASKS_READY','IN_PROGRESS','DELIVERY_SUBMITTED','CI_RUNNING','CI_PASSED','READY_TO_CLOSE','DONE','CANCELLED','FAILED']
const workflowStatusRank=new Map(workflowStatusOrder.map((status,index)=>[status,index]))
const workflowTimestamp=value=>{const timestamp=Date.parse(value||'');return Number.isFinite(timestamp)?timestamp:0}
export function sortWorkflows(workflows,sort='created-desc'){
  const items=Array.isArray(workflows)?[...workflows]:[]
  return items.sort((left,right)=>{
    const createdDifference=workflowTimestamp(right.createdAt)-workflowTimestamp(left.createdAt)
    if(sort==='created-desc')return createdDifference||Number(right.id||0)-Number(left.id||0)
    if(sort==='created-asc')return -createdDifference||Number(left.id||0)-Number(right.id||0)
    const rankDifference=(workflowStatusRank.get(right.status)??-1)-(workflowStatusRank.get(left.status)??-1)
    if(sort==='status-desc')return rankDifference||createdDifference||Number(right.id||0)-Number(left.id||0)
...
export function App(){
  ...
  const projectLeader=project&&(project.createdBy==null||String(project.createdBy)===String(user.userId))
  const nav=[['overview','项目概览',LayoutDashboard],['my-tasks','我的任务',CheckCircle2],['workflows','工作流',WorkflowIcon],['board','任务看板',LayoutDashboard],['members','项目成员',Users],['notifications','通知',Bell],...((!project||projectLeader)?[['audit','审计日志',FileText]]:[]),['archive','归档文件',FolderArchive],...(projectLeader?[['call-logs','调用日志',Activity]]:[])]
  ...
  const pages={
    ...
    board:<Board project={project} onTask={task=>openTask(task,'board')}/>,
...
function Workflows({project,workflows,onCreate,onSelect,onBoard}){
  const [sort,setSort]=useState('created-desc')
  const sortedWorkflows=useMemo(()=>sortWorkflows(workflows,sort),[workflows,sort])
  return <><div className="page-head"><div><h1>工作流</h1><p>{project?'意图、文档版本和交付状态':'请选择项目后查看工作流'}</p></div><div className="workflow-page-actions">{project&&<label className="workflow-sort">排序<select aria-label="工作流排序" value={sort} onChange={event=>setSort(event.target.value)}><option value="created-desc">创建时间：倒序（新到旧）</option><option value="created-asc">创建时间：顺序（旧到新）</option><option value="status-desc">完成度：倒序（高到低）</option><option value="status-asc">完成度：顺序（低到高）</option></select></label>}{project&&<button className="primary" onClick={onCreate} disabled={project.status&&project.status!=='ACTIVE'} title={project.status&&project.status!=='ACTIVE'?'归档项目不能创建工作流':'创建工作流'}><Plus size={16}/>创建工作流</button>}</div></div><section className="panel">{sortedWorkflows.length?sortedWorkflows.map(w=><div className="list-row clickable" key={w.id} onClick={()=>onSelect(w.id)}><div><strong>{w.title}</strong><small>{w.intentLevel} · {w.description}</small><small>Pull Request：{w.intentLevel==='ARCHITECTURE'?'不适用':w.pullRequestRequired===false?'不要求':'必须'}</small></div><Status value={w.status}/><button className="button" onClick={event=>{event.stopPropagation();onBoard(w.id)}}>看板</button></div>):<Empty text={project?'暂无 Workflow':'暂无项目，请先创建项目'}/>}</section></>}
function Board({project,onTask}){
  ...
  useEffect(()=>{
    ...
    api(`/api/projects/${project.id}/board`,{signal:controller.signal}).then(setData).catch(e=>{if(e?.name!=='AbortError')setError(e.message)}).finally(()=>{if(!controller.signal.aborted)setLoading(false)})
    return()=>controller.abort()
  },[project?.id])
  const groups=useMemo(()=>Object.fromEntries((data?.columns||[]).map(c=>[c.key,c.cards])),[data])
  return <><div className="page-head"><div><h1>任务看板</h1><p>{project?`${project.name} · 全部成员 · 只读聚合`:'请选择项目后查看任务看板'}</p></div></div>{error&&<p className="error">{error}</p>}{!project?<Empty text="暂无项目，请先选择项目"/>:loading&&!data?<Empty text="正在加载任务看板"/>:<div className="board-scroll"><div className="board">{columns.map(([key,label])=><section className="column" key={key}><h3>{label}<b>{(groups[key]||[]).length}</b></h3>{(groups[key]||[]).map(t=><article className="task" key={t.id} role="button" tabIndex={0} onClick={()=>onTask?.(t)} onKeyDown={event=>{if(event.key==='Enter'||event.key===' ')onTask?.(t)}}><strong>{t.externalKey||t.taskKey} · {t.title}</strong><Status value={t.status}/><small>负责人：{t.assignee?.username||'未分配'}</small><small>任务包 v{t.currentPackageVersion||'-'} · {t.branchName||'无分支'}</small><small>CI：{t.ciStatus||'--'} · {t.updatedAt||'--'}</small></article>)}{!(groups[key]||[]).length&&<Empty text="暂无任务"/>}</section>)}</div></div>}</>}
...
function WorkflowDetail({workflowId,project,user,onBack,onTask,onUpdated}){
  ...
  },[run?.runId,run?.status,run?.startedAt,run?.createdAt])
  ...
  const savePlan=async nextContent=>{setBusy(true);setError('');try{const content=typeof nextContent==='string'?nextContent:planDraft;JSON.parse(content);await api(`/api/workflows/${workflowId}/plan-drafts`,{method:'PUT',body:JSON.stringify({content})});setPlanEditing(false);await load();return true}catch(e){setError(e instanceof SyntaxError?'Build Plan 不是有效 JSON':e.message);return false}finally{setBusy(false)}}
  ...
  const planNeedsRegeneration=workflow?.status==='BUILD_PLAN_PROPOSED'&&Array.isArray(plan?.assignments)&&plan.assignments.some(assignment=>{
    const member=projectMembers.find(item=>String(item.userId)===String(assignment.userId))
    return member&&((assignment.profileVersion!=null&&member.profileVersion!=null&&Number(assignment.profileVersion)!==Number(member.profileVersion))
      ||(assignment.projectRole&&member.projectRole&&assignment.projectRole!==member.projectRole))
  })
  const regenerationNeeded=planNeedsRegeneration||error.includes('[ASSIGNEE_PROFILE_STALE]')||error.includes('[ASSIGNEE_ROLE_STALE]')
  ...
  return <>
    ...
    {build&&<BuildPlanSection workflow={workflow} document={build} plan={plan} members={projectMembers} warnings={granularity?.warnings||[]} editing={planEditing} draft={planDraft} busy={busy} canApprovePlan={isLeader} planNeedsRegeneration={regenerationNeeded} generationActive={generationActive} onRegeneratePlan={()=>action(`/api/workflows/${workflowId}/generate-build-plan`)} onEdit={()=>{setPlanDraft(formatJson(build.content));setPlanEditing(true)}} onDraft={setPlanDraft} onCancel={()=>setPlanEditing(false)} onSave={savePlan} onApprove={reason=>action(`/api/workflows/${workflowId}/approve-plan`,{versionNo:build.versionNo,reason})} onGranularity={applyGranularity} onCreateTasks={()=>action(`/api/workflows/${workflowId}/create-tasks`)}/>}
    ...
    {isLeader&&<section className="panel"><h2>Workflow 时间线</h2>{audits.length?audits.map(item=><AuditEntry item={item} key={item.id}/>):<Empty text="暂无 Workflow 审计记录"/>}</section>}
  </>
}
...
export function BuildPlanSection({workflow,document,plan,members=[],warnings=[],editing,draft,busy,canApprovePlan,planNeedsRegeneration,generationActive=false,onRegeneratePlan,onEdit,onDraft,onCancel,onSave,onApprove,onGranularity,onCreateTasks}){
  const [selected,setSelected]=useState([]),[reason,setReason]=useState(''),[assignmentEditing,setAssignmentEditing]=useState(false),[assignmentDraft,setAssignmentDraft]=useState(null),[regenerationPending,setRegenerationPending]=useState(false)
  const taskKeys=(plan?.tasks||[]).map(t=>t.taskKey); const hasWarnings=warnings.length>0
  useEffect(()=>{setSelected([]);setReason('');setAssignmentEditing(false);setAssignmentDraft(plan?JSON.parse(JSON.stringify(plan)):null)},[document.versionNo])
  useEffect(()=>{if(generationActive)setRegenerationPending(true);else if(!busy)setRegenerationPending(false)},[generationActive,busy])
  const toggle=key=>setSelected(v=>v.includes(key)?v.filter(x=>x!==key):[...v,key])
  const startAssignmentEdit=()=>{setAssignmentDraft(plan?JSON.parse(JSON.stringify(plan)):null);setAssignmentEditing(true)}
  const updateAssignment=(taskKey,userId)=>setAssignmentDraft(current=>{if(!current)return current;const member=members.find(item=>String(item.userId)===String(userId));return {...current,assignments:(current.assignments||[]).map(item=>item.taskKey===taskKey?{...item,userId:Number(userId),projectRole:member?.projectRole||item.projectRole,profileVersion:member?.profileVersion??item.profileVersion,workloadSnapshot:{...(item.workloadSnapshot||{}),weeklyCapacityPoints:member?.weeklyCapacityPoints??item.workloadSnapshot?.weeklyCapacityPoints,availability:member?.availability??item.workloadSnapshot?.availability},fitReason:`手动分配给 ${member?.username||`User #${userId}`}`}:item)}})
  const saveAssignments=async()=>{if(!assignmentDraft)return;const saved=await onSave(JSON.stringify(assignmentDraft,null,2));if(saved)setAssignmentEditing(false)}
  const assignmentByTask=Object.fromEntries((assignmentDraft?.assignments||[]).map(item=>[item.taskKey,item]))
  return <section className="panel"><div className="section-head"><div><h2>Build Plan</h2><p>v{document.versionNo} · {document.confirmed?'已批准':'待审批'}</p></div>{workflow.status==='BUILD_PLAN_PROPOSED'&&!editing&&<div className="action-row plan-actions">{planNeedsRegeneration&&canApprovePlan&&<button className="button" onClick={()=>{setRegenerationPending(true);onRegeneratePlan?.()}} disabled={busy||generationActive||regenerationPending} title="负责人画像或角色快照已变化"><RefreshCw size={15}/>{generationActive||regenerationPending?'生成中':'重新生成计划'}</button>}{canApprovePlan&&plan?.assignments?.length>0&&<button className="button" onClick={startAssignmentEdit} disabled={busy||assignmentEditing||generationActive||regenerationPending}><Users size={15}/>编辑任务分配</button>}<button className="button" onClick={onEdit} disabled={busy||generationActive||regenerationPending}><Pencil size={15}/>高级 JSON 编辑</button></div>}</div>{editing?<><textarea className="json-editor" value={draft} onChange={e=>onDraft(e.target.value)} spellCheck={false}/><div className="action-row"><button className="button" onClick={onCancel} disabled={busy}>取消</button><button className="primary" onClick={onSave} disabled={busy}>保存新版本</button></div></>:plan?<BuildPlanPreview plan={plan}/>:<pre className="document-source">{document.content}</pre>}{assignmentEditing&&<section className="assignment-editor"><h3>任务分配</h3><p>选择新的负责人后保存，会生成一个新的 Build Plan 版本并刷新负责人画像快照。</p>{(assignmentDraft?.assignments||[]).map(assignment=>{const task=taskKeys.find(key=>key===assignment.taskKey);return <label className="assignment-editor-row" key={assignment.taskKey}><span><strong>{assignment.taskKey}</strong><small>{plan?.tasks?.find(item=>item.taskKey===task)?.title||'任务'}</small></span><select aria-label={`任务 ${assignment.taskKey} 负责人`} value={assignment.userId??''} onChange={event=>updateAssignment(assignment.taskKey,event.target.value)}>{members.map(member=><option key={member.userId} value={member.userId} disabled={!member.profileCompleted}>{member.username||`User #${member.userId}`} · {member.projectRole} · 画像 v{member.profileVersion??0}{member.profileCompleted?'':'（未完成画像）'}</option>)}</select></label>})}<div className="action-row"><button className="button" onClick={()=>setAssignmentEditing(false)} disabled={busy}>取消</button><button className="primary" onClick={saveAssignments} disabled={busy}>保存任务分配</button></div></section>}{workflow.status==='BUILD_PLAN_PROPOSED'&&!editing&&plan?.tasks?.length>1&&<div className="granularity-panel"><h3>任务拆分辅助</h3>{hasWarnings?<>{warnings.map(w=><p key={w.message} className="warning">{w.message}（{(w.taskKeys||[]).join('、')}）</p>)}<p className="warning">保留当前拆分或直接批准此版本时必须填写理由。</p><div className="task-merge-options">{taskKeys.map(key=><label key={key}><input type="checkbox" checked={selected.includes(key)} onChange={()=>toggle(key)}/>{key}</label>)}</div><textarea aria-label="拆分覆盖理由" placeholder="填写保留拆分理由；合并理由可选" maxLength={2000} value={reason} onChange={e=>setReason(e.target.value)}/><div className="action-row"><button className="button" disabled={busy||selected.length<2} onClick={()=>onGranularity({operation:'MERGE',taskKeys:selected,reason})}>合并选中 Task</button><button className="button" disabled={busy||!reason.trim()} onClick={()=>onGranularity({operation:'KEEP_SPLIT',taskKeys:selected.length?selected:taskKeys,reason})}>保留当前拆分</button></div></>:<p>未发现明显的过细拆分。</p>}</div>}<div className="action-row">{workflow.status==='BUILD_PLAN_PROPOSED'&&!editing&&(canApprovePlan?<button className="primary" onClick={()=>onApprove(reason)} disabled={busy||(hasWarnings&&!reason.trim())}>批准 v{document.versionNo}</button>:<span className="plan-waiting" role="status">等待 Leader 批准 Build Plan</span>)}{workflow.status==='PLAN_APPROVED'&&<button className="primary" onClick={onCreateTasks} disabled={busy}>{workflow.intentLevel==='ARCHITECTURE'?'创建并开启子 Workflow':'创建任务'}</button>}</div></section>
}
...
function Notifications({items,onRefresh}){const [error,setError]=useState(''),[busyId,setBusyId]=useState(null);useEffect(()=>{onRefresh?.()},[]);const markRead=async id=>{setBusyId(id);setError('');try{await api(`/api/notifications/${id}/read`,{method:'POST'});await onRefresh?.()}catch(e){setError(e.message)}finally{setBusyId(null)}};return <><div className="page-head"><div><h1>通知</h1><p>任务分配、重新分配、阻塞和交付提醒</p></div><button className="button" onClick={onRefresh}><RefreshCw size={15}/>刷新</button></div>{error&&<div className="alert"><CircleAlert size={16}/>{error}</div>}<section className="panel">{items.length?items.map(n=><div className={`list-row ${n.readAt?'':'unread'}`} key={n.id}><div><strong>{n.title}</strong><small>{n.content} · {formatTime(n.createdAt)}</small>{['TASK_ASSIGNED','TASK_REASSIGNED'].includes(n.type)&&<span className="notification-type">任务分配</span>}{n.type==='WORKFLOW_BUILD_PLAN_APPROVAL_REQUIRED'&&<span className="notification-type">Build Plan 审批</span>}</div>{n.readAt?<Status value="CURRENT"/>:<button className="button" disabled={busyId===n.id} onClick={()=>markRead(n.id)}>{busyId===n.id?'处理中…':'标记已读'}</button>}</div>):<Empty text="暂无通知"/>}</section></>}
function AuditEntry({item}){
  const actor=item.actorUserId==null?'系统':`${item.actorUsername||'未知用户'} (#${item.actorUserId})`
  return <details className="audit-entry"><summary><time>{formatTime(item.createdAt)}</time><span><strong>{item.action||'未知操作'} · {item.entityType||'未知实体'} #{item.entityId??'-'}</strong><small>操作人：{actor}</small></span></summary><dl className="audit-details"><div><dt>审计 ID</dt><dd>{item.id??'-'}</dd></div><div><dt>操作人编号</dt><dd>{item.actorUserId??'系统'}</dd></div><div><dt>操作人用户名</dt><dd>{item.actorUsername||'系统'}</dd></div><div><dt>实体</dt><dd>{item.entityType||'-'} #{item.entityId??'-'}</dd></div><div><dt>记录时间</dt><dd>{formatTime(item.createdAt)}</dd></div><div><dt>详情</dt><dd><pre>{formatJsonValue(item.details)}</pre></dd></div></dl></details>
}
function Audit({project}){const path=project?`/api/audit-logs?projectId=${project.id}&page=0&size=50`:'';const [state]=useProjectCollection(project?.id,path,response=>Array.isArray(response)?response:response?.content);return <><div className="page-head"><div><h1>审计日志</h1><p>项目级时间线，按最新时间排序，仅 Leader 可见</p></div></div><section className="panel">{!project?<Empty text="暂无项目，创建项目后可查看审计日志"/>:state.loading?<Empty text="正在加载审计日志"/>:state.error?<Empty text={`无法加载审计日志：${state.error}`}/>:state.items.length?state.items.map(a=><AuditEntry item={a} key={a.id}/>):<Empty text="暂无审计记录"/>}</section></>}
const archiveCategories=[['ALL','全部文件'],['WORKFLOW_DOCUMENT','Workflow 文档'],['TASK_PACKAGE','Task Package'],['DELIVERY_REPORT','交付报告']]
function Archive({project}){
  const [state,setState]=useState({items:[],loading:false,error:''}),[category,setCategory]=useState('ALL'),[selected,setSelected]=useState(new Set()),[downloading,setDownloading]=useState(false)
  useEffect(()=>{if(!project?.id){setState({items:[],loading:false,error:''});setSelected(new Set());return}const controller=new AbortController();setState({items:[],loading:true,error:''});setSelected(new Set());api(`/api/projects/${project.id}/archive-files`,{signal:controller.signal}).then(items=>setState({items:Array.isArray(items)?items:[],loading:false,error:''})).catch(error=>{if(error?.name!=='AbortError')setState({items:[],loading:false,error:error.message||'归档文件加载失败'})});return()=>controller.abort()},[project?.id])
  const visible=state.items.filter(item=>category==='ALL'||item.category===category)
  const toggle=key=>setSelected(current=>{const next=new Set(current);next.has(key)?next.delete(key):next.add(key);return next})
  const download=async(forceCategory=false)=>{if(!project?.id||downloading)return;setDownloading(true);try{const keys=forceCategory?[]:[...selected];const downloadCategory=keys.length?'ALL':category;const params=new URLSearchParams({category:downloadCategory});keys.forEach(key=>params.append('keys',key));const token=localStorage.getItem('ac_token');const response=await fetch(`/api/projects/${project.id}/archive-files/download?${params}`,{headers:token?{Authorization:`Bearer ${token}`}:{}});if(!response.ok)throw new Error('归档下载失败');const blob=await response.blob();const url=URL.createObjectURL(blob);const anchor=document.createElement('a');anchor.href=url;anchor.download=`archive-${downloadCategory.toLowerCase()}.zip`;anchor.click();setTimeout(()=>URL.revokeObjectURL(url),0)}catch(error){setState(current=>({...current,error:error.message||'归档下载失败'}))}finally{setDownloading(false)}}
  const allVisibleSelected=visible.length>0&&visible.every(item=>selected.has(item.key))
  const toggleVisible=()=>setSelected(current=>{const next=new Set(current);visible.forEach(item=>allVisibleSelected?next.delete(item.key):next.add(item.key));return next})
  return <><div className="page-head"><div><h1>归档文件</h1><p>集中归档 Workflow 文档、任务包和交付报告，可按分类批量下载</p></div><div className="archive-actions"><button className="button" disabled={!visible.length||downloading} onClick={()=>download(true)}><Download size={16}/>下载当前分类</button><button className="primary" disabled={!selected.size||downloading} onClick={()=>download(false)}><Download size={16}/>{downloading?'正在打包…':`下载已选（${selected.size}）`}</button></div></div>{state.error&&<div className="alert"><CircleAlert size={16}/>{state.error}</div>}<div className="archive-tabs" role="tablist">{archiveCategories.map(([key,label])=><button key={key} className={category===key?'active':''} role="tab" aria-selected={category===key} onClick={()=>setCategory(key)}>{label} ({state.items.filter(item=>key==='ALL'||item.category===key).length})</button>)}</div><section className="panel archive-panel">{!project?<Empty text="暂无项目，请先选择项目"/>:state.loading?<Empty text="正在加载归档文件"/>:visible.length===0?<Empty text="当前分类暂无归档文件"/>:<><label className="archive-select-all"><input type="checkbox" checked={allVisibleSelected} onChange={toggleVisible}/>全选当前分类</label>{visible.map(item=><label className="archive-row" key={item.key}><input type="checkbox" checked={selected.has(item.key)} onChange={()=>toggle(item.key)}/><span><strong>{item.title||item.fileName}</strong><small>{item.fileName} · {item.contentFormat||'TEXT'} · {Number(item.size||0).toLocaleString()} bytes · {formatTime(item.createdAt)}</small></span></label>)}</>}</section></>}
